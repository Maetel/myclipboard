...
같은 텍스트나 링크를 여러 번 복사한 경우에는 가장 최근 기록 하나만 표시합니다. 파일과 이미지는 이름이 같아도 서로 다른 내용일 수 있으므로 각각 표시합니다.

비밀번호는 기기에 저장하지 않습니다. Windows에서는 로그인 세션과 로컬 기록 암호화 키를 운영체제 보안 저장소에 보관합니다. macOS에서는 앱을 다시 빌드할 때마다 Keychain이 사용자 로그인 비밀번호를 요구하지 않도록, 앱 전용 사용자 폴더 안의 소유자 전용 디렉터리와 파일(`0700`/`0600`)에 보관합니다. 로컬 기록과 전송 대기 항목은 AES-256-GCM으로 암호화합니다. 서버는 계정별 기록을 분리하고 7일 또는 최근 500개까지만 보관합니다. 앱에 남길 최근 기록 개수는 설정에서 1~500개로 정하며 기본값은 200개입니다.
...
Windows 앱은 운영체제의 클립보드 변경 알림으로 새 복사 항목을 감지하고, 로그인한 동안 2초마다 마이메모 Clipboard feed의 변경분을 확인합니다. macOS도 같은 주기로 확인하므로 한 기기에서 생긴 기록이 다른 기기에 거의 동시에 나타납니다. 시작할 때는 WebView2를 만들지 않고, 설정이나 클립보드 기록을 실제로 열 때만 생성합니다. 창을 닫으면 WebView2도 제거하며 동기화가 끝난 뒤 유휴 HTTP 연결을 유지하지 않습니다.
...
macOS에서는 저장소의 `MyMemo Clipboard Dev.command`, Windows에서는 `MyMemo Clipboard Dev.cmd`를 더블클릭하면 앱을 개발 모드로 바로 실행합니다. 실행기는 30초마다 `main`의 최신 코드를 `git pull --ff-only`로 확인하고, 새 commit을 받으면 앱을 안전하게 다시 시작합니다. `package.json`이나 lockfile이 바뀐 경우에는 의존성도 먼저 맞춥니다. 로컬에서 수정 중인 파일이 있거나 다른 브랜치에 있으면 파일을 덮어쓰지 않고 자동 업데이트만 건너뜁니다. 네트워크 문제로 업데이트하지 못해도 이미 받아 둔 코드로 계속 실행합니다.

처음 한 번만 전체 의존성을 설치하고, 이후 프런트엔드와 Rust 코드 변경을 감지해 자동으로 다시 반영합니다. 터미널에서는 다음 명령과 같습니다.

```bash
npm run dev:self-update
```
