...
macOS에서는 기록·썸네일·파일 전송과 Keychain 확인을 화면 처리와 분리해, 느린 작업 중에도 창과 Escape 입력이 반응하도록 구성합니다. 저장된 로그인 정보가 있으면 로그인 화면을 먼저 보여 주지 않고 계정 화면으로 이동합니다. 파일은 문자열 경로가 아닌 macOS 파일 목록 Clipboard 형식으로 공유합니다.
